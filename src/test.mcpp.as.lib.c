/*
 * test.mcpp.as.lib.c: A sample source to show how to use mcpp as a subroutine.
...
int main(int argc, char *argv[])
{
    ...
    for (j = i; i < argc; ++i) 
    {
        ...
        result = mcpp_get_mem_buffer(OUT); 
        if (result)
            fputs( result, stdout);
 
        /* get the diagnostics  */   
        
        result = mcpp_get_mem_buffer(ERR); 
        if (result)
            fputs( result, stderr);
				...
				result = mcpp_get_mem_buffer(DBG);
				if (result)
